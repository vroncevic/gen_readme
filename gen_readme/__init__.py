    ...
    def __init__(self, verbose: bool = False) -> None:
        ...
        if self.is_operational():
            self.add_new_option(
                ...
                help='generate readme (provide project name)'
            )
            self.add_new_option(
                self._OPS[2], self._OPS[3], dest='type',
                help='type (avr/c/cc/js/pl/py/rpi/sh/stm/vala)'
            )
            self.add_new_option(
        ...
        if self.is_operational():
            try:
                ...
                if exists(f'{getcwd()}/{str(getattr(args, "name"))}'):
                    ...
                    return status
                gen: GenReadmeModule = GenReadmeModule(
                ...
                try:
                    print(
                        " ".join([
                            f'[{self._GEN_VERBOSE.lower()}]',
                            'generate README module skeleton',
