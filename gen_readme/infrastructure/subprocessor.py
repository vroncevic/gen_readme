...
class SubProcessor:
    ...
    def run(self, *, params: Mapping[str, object]) -> Mapping[str, object]:
        ...
        try:
            ...
            if success and filename != 'README.md':
                os.rename(join(output_dir, 'README.md'), join(output_dir, filename))
            ...
            return {
                ...
                'stdout': f'{filename} successfully generated.' if success else '',
                'stderr': f'failed to generate {filename}.' if not success else ''
            }

        except Exception as exc:
            return {'returncode': 1, 'stdout': '', 'stderr': f'failed to generate README {exc}'}
